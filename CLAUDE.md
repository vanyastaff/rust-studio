...
`.autopilot/` **не коммитится** (с 0.51.0) — это рабочая папка одного прогона, она живёт
только локально. В свежем клоне её не будет, и это нормально: всё, что должно пережить
прогон, переносится в `plugins/rust-studio/docs/adr/` — так и появился ADR 0001.
<!-- autopilot:end -->
