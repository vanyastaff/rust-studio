...
It should also flag at least one of: `Frame` is not `#[repr(C)]` and contains a `Vec` yet crosses the boundary (lines 15–19); edition 2024 requires `unsafe extern "C"` and `#[unsafe(no_mangle)]` (lines 21, 26, 34, 40, 49); undocumented null/safety contracts on raw pointers (lines 27, 41, 50); `len() as c_int` truncation (line 51).
