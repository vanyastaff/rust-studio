...
7. Review the diff for ASYNC-GATE compliance. Navigate code with the session's language-server layer (harness `LSP` tool or serena, per `${CLAUDE_PLUGIN_ROOT}/docs/tooling.md`)
   and `rg`/`cargo clippy` — not Bash grep.
