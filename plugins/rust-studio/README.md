...
- **Measured, not asserted** — every eval case and every agent fixture runs over the headless CLI with the plugin loaded (`tools/eval-runner.ts`), so a prompt edit is scored before it ships. The 0.44.0 run: 31 cases, 42 fixtures, 18 agents; the numbers and the misses are in the CHANGELOG, and a miss is filed against the agent's brief — or, twice in that run, against a rule that turned out to be wrong
