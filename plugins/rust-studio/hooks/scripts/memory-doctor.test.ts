...
describe("vault import", () => {
  const vaultNote =
    '---\ntitle: "A retry budget has two obligations"\ntags: [gotcha, http-client]\nnote_type: gotcha\nstatus: active\ncreated: 2026-08-07\nupdated: 2026-08-10\n---\n\nCounting attempts is half the job. See [[backoff-jitter|Backoff needs jitter]] and [[reset-on-success]].\n';
  ...
  test("convertVaultNote produces host frontmatter, keeps dates, maps kind → type", () => {
    const c = convertVaultNote("retry-budget", vaultNote, "count attempts AND reset on success", NOW, "acme/retry-budget.md");
    expect(c.file).toBe("retry-budget.md");
    expect(c.content).toContain("name: retry-budget");
    expect(c.content).toContain('description: "A retry budget has two obligations"');
    ...
    expect(c.content).toContain("[Backoff needs jitter](backoff-jitter.md)");
    expect(c.content).not.toContain("note_type");
    expect(c.indexLine).toBe("- [A retry budget has two obligations](retry-budget.md) — count attempts AND reset on success");
  ...
  test("planImport + applyImport: creates, indexes, never overwrites, and skips on re-run", () => {
    ...
    mkdirSync(vault);
    writeFileSync(join(vault, "retry-budget.md"), vaultNote);
    writeFileSync(join(vault, "already.md"), '---\ntitle: "Already"\n---\nold text');
    writeFileSync(join(vault, "empty.md"), "   ");
    writeFileSync(join(vault, "MEMORY.md"), "# acme — project memory\n\n- [[retry-budget|A retry budget has two obligations]] — count attempts\n");
    writeFileSync(join(mem, "already.md"), "keep me");
    const plan = planImport(vault, mem, NOW);
    expect(plan.create.map((c) => c.file)).toEqual(["retry-budget.md"]);
    ...
    expect(parseIndex(index).map((e) => e.file)).toEqual(["retry-budget.md"]);
    expect(index).toContain("— count attempts");
    const again = planImport(vault, mem, NOW);
    expect(again.create).toHaveLength(0);
    expect(again.skipExists).toContain("retry-budget.md");
  });
});
...
describe("indexTitle", () => {
  test("keeps short titles, cuts a long one at its first clause break", () => {
    ...
    expect(indexTitle("A retry budget has two paired obligations — count attempts AND reset on success, or you get one-shot work")).toBe(
      "A retry budget has two paired obligations",
    );
  });
