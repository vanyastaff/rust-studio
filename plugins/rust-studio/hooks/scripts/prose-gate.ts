...
// - `audit` takes already-stripped text. The caller picks `stripProse` (markdown) or
//   `rustdocProse` (a .rs file); the CLI will do that by extension.
//
// Zero dependencies (bun, no imports beyond _lib.ts) so the file ships inside the /prose
// bundle. This version is the library; the CLI is the next task, and the `import.meta.main`
// block at the bottom only says so.
...
/** `.`, `!` or `?` (a run of them), any closing quote, bracket or emphasis marker, then
 *  whitespace. The whitespace is a lookahead so the next sentence starts after it. */
 ...
 *  This is the safe direction: the blank a stripped span leaves, a dash, or a capital all
 *  end the sentence, and a split can only hide a dash pair, never invent one. */
const ETC = /(?:^|[\s(])etc\.$/i;
 ...
 *  bounded window around the candidate (8 characters back, 16 forward) so a block with many
 ...
 *  judges a whole file by a few changed sentences. */
...
if (import.meta.main) {
  process.stderr.write(
    "usage: prose-gate.ts is the linter library in this version; the CLI (--full, --since, --stdin, --json) lands in the next task\n",
  );
  process.exit(2);
}
