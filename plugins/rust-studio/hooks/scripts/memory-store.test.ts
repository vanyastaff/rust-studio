...
describe("projectKey", () => {
  test("mirrors the host: every char outside [A-Za-z0-9-] becomes '-'", () => {
    expect(projectKey("/home/u/proj/rust-studio")).toBe("-home-u-proj-rust-studio");
...
describe("parseIndex", () => {
  ...
  test("legacy vault form: - [[slug|Title]] — hook resolves to slug.md", () => {
    const [e] = parseIndex("- [[acme-layer-drift|AGENTS.md edition note is stale]] — check Cargo.toml first");
    expect(e).toMatchObject({ title: "AGENTS.md edition note is stale", file: "acme-layer-drift.md", hook: "check Cargo.toml first" });
  });
