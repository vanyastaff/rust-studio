...
|------|------|--------------|----------|--------|
| GT-1 | 12   | WRONG-CRATE  | 🟣 | `compute_order_total` is Order/Money domain math living in `acme-http`. It belongs in the crate that owns Order (`acme-domain`); it was added here only because the handler was. Move it. |
