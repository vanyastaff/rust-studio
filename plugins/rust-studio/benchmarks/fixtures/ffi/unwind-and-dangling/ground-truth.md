...
| GT-4 | 15–19, 31 | OPAQUE-HANDLE CONTRACT UNSTATED | 🟡 | `Frame` is `repr(Rust)` and contains a `Vec`, yet `*mut Frame` crosses the boundary. That is sound **only** as an opaque handle: the header must declare `Frame` opaque, C must never read its fields, and every read goes through an accessor (`acme_frame_len` is one). Nothing here states that contract, so a binding author can legitimately reach for `frame->width`. Say it, or hand C a `#[repr(C)]` view struct without the `Vec`. |
