...
| GT-3 | 23–25  | UNHYGIENIC LOCAL / RESERVED NAME | 🟠 | `traced!` introduces `let tmp` — hygiene protects the caller's `tmp` from being captured, but a caller passing an expression that *mentions* their own `tmp` gets the macro's binding (item/local scoping trap). Use an unlikely name (`__traced_value`) or document the reserved identifier. |
