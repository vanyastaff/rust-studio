...
A change to a shared `nebula-core` enum must ripple to EVERY workspace member. One
consumer was updated; the other compiles but silently mishandles the new case.

| id   | line | type              | severity | defect |
|------|------|-------------------|----------|--------|
| GT-1 | 31   | INCOMPLETE-RIPPLE | 🔴 | `nebula-worker::dispatch` still has a catch-all `_ => Ok(())` that silently drops the new `Job::Push` — jobs are accepted and never sent. The cross-crate change did not update this member. |
