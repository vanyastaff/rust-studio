...
failures in a crate that passes under the gate, because the gate supplies `APP_HEADLESS=1` and
...
3. **Copy the exact invocation to narrow a failure** — feature flags, `--no-default-features`,
   target triple, env vars (`RUSTFLAGS`, `APP_HEADLESS=1`), and wrappers (`xvfb-run`) included.
