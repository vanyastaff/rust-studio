...
`references/integrity-and-evidence.md`, governs the **honesty of the result and
...
are codified in focused `references/` files; every touched line must satisfy them:
...
The pre-code standard is reinforced by focused `references/` files:
