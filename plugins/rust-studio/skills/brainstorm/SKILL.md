...
3. Record the clarified goal and constraints as a short header before the options. Carry it
   into the handoff at step 11 so a later `/spec` can preserve the decision without reopening
   the interview. Revise it if the conversation shifts.
...
11. On approval, **delegate saving the concept note** to `product-steward` — do not write
    files directly. Hand over the step-3 goal-and-constraints header with it, so a later
    `/spec` can reuse it without reopening the interview. Instruct `product-steward` to store it at the path it deems appropriate
